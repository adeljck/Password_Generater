...
def deal_data(data: dict) -> tuple:
    ...
    """
    # name，姓名的各种情况
    get_low_and_up_str = lambda l: [j for i in l if i for j in [i.lower(), i.upper()]]
    get_repeat = lambda l, x: l + [i * 2 for i in l if 0 < len(i) <= x]
    get_head_tail = lambda s, *l: [j for i in l if len(s) > i for j in [s[:i], s[-i:]]]
    distinct_list = lambda l: [i for i in set(l) if i]
    ...
    name_all.extend(
        [first_name[0:1] + last_name_a_b, first_name_combine[0] + last_name_a_b, last_name_a_b + first_name[0:1],
    ...
    id_card_all = [] if len(id_card) != 18 else distinct_list(
        get_repeat(get_head_tail(id_card, 3, 4, 6, 8) + get_head_tail(id_card[:-1], 3, 4, 6, 8)))
    # work_no，工号
...
def index_get():
    get_capitalize = lambda l: [j for i in l if i for j in [i, i.capitalize()]]
    drop_short_long = lambda l, start=6, end=16: [x for x in l if start <= len(x) <= end]
    distinct_list = lambda l: [i for i in set(l) if i]
...
def get_common():
    ...
    with open(os.path.join(dirname, 'static', 'file', 'NCSC_PwnedPasswordTop100k.txt'), 'r', encoding='utf8') as f:
        NCSC100k = f.read()
    return jsonify(
        {'content100': content100, 'content1700': content1700, 'content10000': content10000, 'NCSC100k': NCSC100k})
